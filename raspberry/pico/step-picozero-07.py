...
このプログラムは、Raspberry Pi を使用して信号機の動作をシミュレートします。
各色のLED（赤、青、黄）をGPIOピンに接続し、指定されたパターンに基づいて点灯および点滅を制御します。
さらに、青信号が点灯中にカッコー音を再生する機能を備えています。

機能:
1. 信号機の色（赤、青、黄）ごとに点灯パターンを定義可能。
2. 青信号の点灯中にカッコー音を再生。
3. 赤信号がデフォルトの状態で、指定されたパターンに従って動作。
4. Ctrl+C を押すことでプログラムを安全に終了。

必要なライブラリ:
- picozero
- cuckoo_signal_active (カッコー音制御用のクラス)

実行方法:
1. 配線を正しく接続します。
2. このスクリプトを実行します。
3. プログラムは信号機の点灯パターンを繰り返し再生します。
4. Ctrl+C を押すとプログラムを終了し、すべてのリソースを解放します。
"""
