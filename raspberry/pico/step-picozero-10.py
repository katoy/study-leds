...
# GPIO 19 --- 同様の接続
# GPIO 20 --- ブザーの制御ピン
# GPIO 16 --- ボタン入力ピン（プルアップ接続）
# 抵抗（330Ω または 1kΩ）

# 信号機シミュレーションプログラム
# 機能:
# - 初期状態で赤信号が点灯し、ボタンの押下を待機します。
# - 赤信号が点灯している間にボタンが押されると、以下の動作を行います。
#   - ブザーを短く鳴らし、2秒後に赤信号を消灯します。
#   - 青信号が点灯し、カッコー音が再生されます（4秒間）。
#   - 青信号が点滅します（0.5秒間隔で4回）。
#   - 黄信号が点灯します（2秒間）。
#   - その後、再び赤信号が点灯し、ボタンの押下を待機します。
# - ボタンが押されない限り、赤信号は点灯し続けます。
#
# 必要なライブラリ:
# - picozero
# - cuckoo_signal_active.py（カッコー音を再生するためのモジュール）
